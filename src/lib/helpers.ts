import { ColumnData, DataType, DisplayFormat, RowData } from "../types/types";
...
const getColumns = (row: RowData, columnNames: string[]): ColumnData[] => {
  ...
  columnNames.map((column) => {
    ...
    columns.push({ name: column, dataType: type, format: DisplayFormat.None });
  });
 ...
 */
export function inferDataTypeAndFormat(value: any): { type: DataType; format: DisplayFormat } {
  ...
  if (typeof value === 'string') {
    ...
    if (dateRegex.test(trimmedValue) && !isNaN(new Date(trimmedValue).getTime())) {
 ...
 */
export function castValueToType(value: any, targetType: DataType): any {
